...
function ab_showFeedsPage()
{
	...
	if (sizeof($feeds) > 0) {
		...
		foreach ($feeds as $feed) {
			...
			echo '&nbsp;<a href="'.$baselink.'&amp;p=&amp;action=run&_fid='.$feed['id'].'"><img style="vertical-align: middle;" src="'.ab_pluginURL().'/img/bprocess.png" /></a>&nbsp;';
			echo '<br/></div></div>';
		} // foreach
		echo '<br/><br/><br/><br/>';
	} else {
	...
	function ab_MetaEditFeedSidebar($feed = '') 
	{
			...
			if (function_exists('wp_nonce_url')) $navlink = wp_nonce_url($_SERVER['PHP_SELF'].'?page='. $_GET['page'], 'autoblogged-nav');
				...
				if (!empty($_REQUEST['_fid'])) {
					...
					echo '<a href="http://viewer.autoblogged.com/?feed='.urlencode(ab_getFeedURL($feed['type'], $feed['url'])).'&type=xml&amp;TB_iframe=true&amp;height=600&width=800" class="thickbox" target="_blank">View Feed XML</a>';
				}
