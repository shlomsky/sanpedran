...
class MnglBoardsController
{
  ...
  {
    global $mngl_options, $mngl_friend, $mngl_user, $mngl_app_helper, $mngl_board_post, $mngl_board_comment, $current_user;
    ...
    if($user)
    {
      ...
      require_once MNGL_VIEWS_PATH . "/mngl-boards/display.php";
    }
  }
  
  function activity_display($user_id, $page=1, $page_size=15)
  {
    global $mngl_friend, $mngl_user, $mngl_app_helper, $mngl_board_post, $mngl_board_comment, $current_user;
  ...
  {
    global $mngl_options, $mngl_friend, $mngl_user, $mngl_app_helper, $mngl_board_post, $mngl_board_comment, $current_user, $mngl_boards_controller;
  ...
  {
    global $mngl_friend, $mngl_user, $mngl_board_post;

    if( MnglUser::is_logged_in_and_visible() and
        ( ($owner_id==$author_id) or
          $mngl_friend->is_friend($owner_id, $author_id) ) )
      $mngl_board_post->create($owner_id, $author_id, strip_tags(stripslashes(MnglAppHelper::decode_unicode($message))));
      
    if( MnglUser::is_logged_in_and_visible() and ($owner_id==$author_id) )
      $mngl_user->update_status(strip_tags(stripslashes(MnglAppHelper::decode_unicode($message))));
  ...
  {
    global $mngl_friend, $mngl_board_comment, $mngl_board_post, $mngl_user;
    ...
    if( MnglUser::is_logged_in_and_visible() and
      ...
      $comment_id = $mngl_board_comment->create($board_post_id, $author_id, strip_tags(stripslashes(MnglAppHelper::decode_unicode($message))));
  ...
  {
    global $mngl_board_post, $mngl_user;
  ...
  {
    global $mngl_board_comment, $mngl_board_post, $mngl_user;
