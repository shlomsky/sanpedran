...
class MnglFriend
{
  ...
  function get_all_by_user_id( $user_id, $where='', $order_by='', $limit='' )
  {
    ...
    $query = "SELECT wpmf.* FROM {$mngl_friend->table_name} wpmf JOIN {$wpdb->prefix}users wpu ON wpmf.friend_id=wpu.ID WHERE wpmf.user_id={$user_id}{$inv_str}{$where}{$order_by}{$limit}";
  ...
  function get_friends_user_array( $user_id, $where='', $offset=0, $limit=50, $order_by='' )
  {
    ...
    $friends = MnglFriend::get_all_by_user_id( $user_id, "status='verified'{$where}", $order_by, "{$offset},{$limit}" );
  ...
  function get_friend_count( $user_id, $where='' )
  {
    ...
    $query = "SELECT COUNT(*) FROM {$mngl_friend->table_name} wpmf JOIN {$wpdb->prefix}users wpu ON wpmf.friend_id=wpu.ID WHERE wpmf.user_id={$user_id}{$inv_str}{$where}";
