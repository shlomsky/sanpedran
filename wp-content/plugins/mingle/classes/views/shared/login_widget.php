      <?php if (MnglUtils::is_user_logged_in()) {
        ...
        <p style="font-size: 10px" class="mngl-logout-button"><a href="<?php echo wp_logout_url(get_permalink($mngl_options->activity_page_id)); ?>"><?php _e('Logout', 'mingle'); ?></a></p>
        <?php
      } else { ?>
      ...
      ?>
        <a href="<?php echo $mngl_blogurl; ?>/wp-login.php?action=lostpassword"><?php _e('Lost Password?', 'mingle'); ?></a>
<?php } ?>
        </p>
      <?php
