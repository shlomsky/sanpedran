...
if(isset($_POST['edit_save_group'])){
	...
	if($_POST['edit_group_name'] == "")
		echo "<div id='message' class='updated fade'><p>".__("You must specify a group name", "wpforum")."</p></div>";
		
	$sql = "SELECT id FROM ".$table_prefix."forum_groups WHERE name = '$edit_group_name' AND id <> $edit_group_id";
	$name = $wpdb->get_var($sql);
	if($name)
		echo "<div id='message' class='updated fade'><p>".__("You have choosen a name that already exists in the database, please specify another", "wpforum")."</p></div>";
		...
		echo "<div id='message' class='updated fade'><p>".__("Group updated successfully", "wpforum")."</p></div>";
	}
}
if(isset($_POST['edit_save_forum'])){
	...
	if($edit_forum_name == "")
		echo "<div id='message' class='updated fade'><p>".__("You must specify a forum name", "wpforum")."</p></div>";
	...
	echo "<div id='message' class='updated fade'><p>".__("Forum updated successfully", "wpforum")."</p></div>";
...
if(($_GET['do'] == "editgroup") && (!isset($_POST['edit_save_group']))){
	...
	$group_name = stripslashes($wpforum->get_groupname($_GET['groupid']));
	global $wpdb, $table_prefix;
	$table = $table_prefix."forum_groups";

	echo "<h2>".__("Edit category", "wpforum")." \"$group_name\"</h2>";
	...
	echo "<table class='form-table'>
			...
			<tr>
				...
				<td><textarea name='edit_group_description' ".ADMIN_ROW_COL.">".stripslashes($wpforum->get_group_description($_GET['groupid']))."</textarea></td>
			</tr>
			<tr>
				<th>".__("User Groups:", "wpforum")."</th>
				<td>";
					
						echo "<strong>".__("Members of the checked User Groups have access to the forums in this category:", "wpforum")."</strong>";
						if($usergroups){
							$i = 0;
							...
							foreach($usergroups as $usergroup){
								$col = 4;
								if($wpforum->array_search($usergroup->id, $usergroups_with_access))
							...
							echo __("There are no User Groups", "wpforum");	
			...
			<tr>
				<th></th>
				<td><input type='submit' name='edit_save_group' value='".__("Save group", "wpforum")."' /></td>
			</tr>
...
if(($_GET['do'] == "editforum") && (!isset($_POST['edit_save_forum']))){
	...
	echo "<h2>".__("Edit forum", "wpforum")." \"".stripslashes($wpforum->get_forumname($_GET['forumid']))."\"</h2>";
	echo "<form id='edit_forum_form' name='edit_forum_form' action='' method='post'>";
	
	echo "<table class='form-table'>";
	echo "<tr>
			<th>".__("Name:", "wpforum")."</th>
			<td><input type='text' name='edit_forum_name' value='".stripslashes($wpforum->get_forumname($_GET['forumid']))."' /></td>
		</tr>
		<tr>
			<th>".__("Description:", "wpforum")."</th>
			<td><textarea name='edit_forum_description' ".ADMIN_ROW_COL.">".stripslashes($wpforum->get_forum_description($_GET['forumid']))."</textarea></td>
		</tr>
		<tr>
			<th></th>
			<td><input type='submit' name='edit_save_forum' value='".__("Save forum", "wpforum")."' /></td>
		</tr>
