...
if(isset($_POST['edit_usergroup_submit'])){
	...
	if(!$edit_usergroup_name)
		echo "<div id='message' class='updated fade'><p>".__("You must specify a name for the User Group", "wpforum")."</p></div>";
		...
		echo "<div id='message' class='updated fade'><p>".__("User Group updated successfully", "wpforum")."</p></div>";
	}
}


else{
	$name = $wpforum->get_usergroup_name($_GET['usergroup_id']);
	echo "<h2>".__("Edit User Group", "wpforum"). " \"$name\"</h2>";
	...
	echo "<table class='form-table'>
			...
			<tr>
				...
				<td><textarea name='edit_usergroup_description' ".ADMIN_ROW_COL.">".$wpforum->get_usergroup_description($_GET['usergroup_id'])."</textarea></td>
			</tr>
			<tr>
				<th></th>
				<td><input type='submit' name='edit_usergroup_submit' value='".__("Save User Group", "wpforum")."'</td>
			</tr>
